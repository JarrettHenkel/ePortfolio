...
namespace
{
    ...
    // Orbit parameters
    const float sphereOrbitSpeed = 0.5f;
    const float sphereOrbitRadius = 5.0f;

    const float cylinderOrbitSpeed = 0.75f;
    const float cylinderOrbitRadius = 3.0f;

    const float torusOrbitSpeed = 2.0f;
    const float torusOrbitRadius = 1.0f;
}
...
void URender()
{
    ...
    // Sphere orbiting the cube
    glm::mat4 sphereModel = glm::mat4(1.0f);
    float sphereAngle = currentTime * sphereOrbitSpeed;
    glm::vec3 spherePosition = glm::vec3(sin(sphereAngle) * sphereOrbitRadius, 0.5f, cos(sphereAngle) * sphereOrbitRadius);
    sphereModel = glm::translate(sphereModel, spherePosition);
    sphereModel = glm::rotate(sphereModel, currentTime * glm::radians(30.0f), glm::vec3(1.0f, 1.0f, 0.0f));
    ...
    glDrawElements(GL_TRIANGLES, gSphere.nIndices, GL_UNSIGNED_INT, 0);

    // Cylinder orbiting the cube
    glm::mat4 cylinderModel = glm::mat4(1.0f);
    float cylinderAngle = currentTime * cylinderOrbitSpeed;
    glm::vec3 cylinderPosition = glm::vec3(sin(cylinderAngle) * cylinderOrbitRadius, 0.0f, cos(cylinderAngle) * cylinderOrbitRadius);
    cylinderModel = glm::translate(cylinderModel, cylinderPosition);
    cylinderModel = glm::rotate(cylinderModel, currentTime * glm::radians(100.0f), glm::vec3(2.0f, 2.0f, 1.0f));
    cylinderModel = glm::scale(cylinderModel, glm::vec3(0.5f, 0.3f, 0.5f));
    ...
    glDrawElements(GL_TRIANGLES, gCylinder.nIndices, GL_UNSIGNED_INT, 0);

    // Torus orbiting the sphere
    glm::mat4 torusModel = glm::mat4(1.0f);
    float torusAngle = currentTime * torusOrbitSpeed;
    glm::vec3 torusOrbitCenter = spherePosition;
    glm::vec3 torusPosition = glm::vec3(sin(torusAngle) * torusOrbitRadius + torusOrbitCenter.x, torusOrbitCenter.y, cos(torusAngle) * torusOrbitRadius + torusOrbitCenter.z);
    torusModel = glm::translate(torusModel, torusPosition);
    torusModel = glm::rotate(torusModel, currentTime * glm::radians(180.0f), glm::vec3(0.0f, 3.0f, 3.0f));
    torusModel = glm::scale(torusModel, glm::vec3(0.2f, 0.2f, 0.5f));
    glBindTexture(GL_TEXTURE_2D, gTorus.texture);
    glUniformMatrix4fv(glGetUniformLocation(gProgramId, "model"), 1, GL_FALSE, glm::value_ptr(torusModel));
    glBindVertexArray(gTorus.vao);
    glDrawElements(GL_TRIANGLES, gTorus.nIndices, GL_UNSIGNED_INT, 0);
